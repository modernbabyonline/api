...
// CorsOptions - is struct that defined cors properties
...
// Handler - the struct that has the CORS options
type Handler struct {
...
// NewCorsHandler - call when wanting to set CORS options
...
// CorsMiddleware - the CORS middlware call to fasthttp
func (c *Handler) CorsMiddleware(innerHandler fasthttp.RequestHandler) fasthttp.RequestHandler {
...
func (c *Handler) handlePreflight(ctx *fasthttp.RequestCtx) {
...
func (c *Handler) handleActual(ctx *fasthttp.RequestCtx) {
	originHeader := string(ctx.Request.Header.Peek("Origin"))
	if !c.isAllowedOrigin(originHeader) {
		c.logger.Log("Origin ", originHeader, " is not in", c.allowedOrigins)
		return
...
func (c *Handler) isAllowedOrigin(originHeader string) bool {
...
func (c *Handler) isAllowedMethod(methodHeader string) bool {
...
func (c *Handler) areHeadersAllowed(headers []string) bool {
